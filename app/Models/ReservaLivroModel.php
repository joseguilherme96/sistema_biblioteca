...
class ReservaLivroModel extends Model
{
    ...
    public function pesquisarReserva($where = [])
    {
        ...
        $reserva = DB::table($this->table)
            ...
            ->join('nivel_acesso','users.nivel_acesso_id','=','nivel_acesso.id_nivel_acesso')
            ->join('status', 'status.id_status', '=', 'reserva_livro.status_id')
            ->select('reserva_livro.*', 'users.name', 'status.descricao as status','nivel_acesso.nome_nivel_acesso')
